...
# Listas extraídas dos CSVs (Usadas nos filtros)
...
def main():
    ...
    st.markdown("Os filtros abaixo comunicam-se diretamente com o servidor (Elasticsearch), garantindo máxima velocidade de resposta mesmo para milhões de linhas.")
    st.markdown("---")
    ...
    with st.expander("🎯 Filtros Opcionais de Performance (Direto na Fonte)", expanded=True):
        ...
        with f1:
            projeto_input = st.multiselect("Projeto:", options=LISTA_PROJETOS, default=["TODOS"])
        with f2:
            atividade_input = st.multiselect("Atividade:", options=LISTA_ATIVIDADES, default=["TODOS"])
        with f3:
    ...
    # Fluxo pós-carregamento para download
    if 'df_visitas_otim' in st.session_state and not st.session_state['df_visitas_otim'].empty:
        df_exibir = st.session_state['df_visitas_otim']
        
        st.markdown("---")
        st.markdown(f"**Visualização Amostral** (Exibindo até 100 de {len(df_exibir):,} registros retornados)")
        ...
        st.download_button(
            label=f"📥 Baixar Relatório em CSV ({len(df_exibir):,} linhas)",
            data=buffer,
            file_name=f"relatorio_visitas_otimizado_{dt_inicio}_a_{dt_fim}.csv",
            mime="text/csv",
